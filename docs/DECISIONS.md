...
- 結果: `fetch` やフォームで PDF を送らない。クラウドへは永続化しない。端末内のクラッシュ保護は D142
...
- 状態: 改訂（D142）
- 判断: supabase-js が Auth session をブラウザ storage に置くことは許可する。制作データの localStorage は置かない
- 理由: リロード後に毎回ログインさせると、OTP のたびにメール待ちになる
- 採用しなかった案: 毎回必ずログイン画面へ戻す。制作データを Auth に紐づけてクラウド保存する
- 結果: リロード → 既存 session → access 再確認 → allowed ならアプリへ進む。制作ドラフトの IndexedDB は D142（Auth session とは別ストア）
...
- 結果: 最短公開ルートは M11.7（済）→ M11.8 → 正式有料公開。M11.5 はこの経路に入れない

## D138. 法務ページは静的 HTML とし、Gate の外に置く

- 状態: 採用（M11.7・COMPLETE）
- 判断: `legal/` に独立した HTML を置く。`index.html` 内のパネルや hash ルートにはしない。Account の「解約・表記」は `legal/index.html` へ行く
- 理由: Auth Gate が `index.html` を覆う。未ログイン閲覧と直 URL を Gate に遮断させない
- 採用しなかった案: アプリ内モーダル。共通 footer を全画面に常設する
- 結果: ログインにプライバシー / 規約。denied の購入直前に規約 / プライバシー / 特商法 / 解約について。Stripe Checkout / Portal への規約 URL は M11.8

## D139. 特商法の氏名・住所・電話は請求開示とし、料金は税込月額 100 円とする

- 状態: 採用（M11.7・COMPLETE。請求開示方式はオーナー方針）
- 判断: 販売事業者名・所在地・電話番号は公開 HTML へ直接掲載せず、「請求があった場合、遅滞なく開示します」と書く。開示請求は公開問い合わせ先へ。公開料金は月額 100 円（税込）。契約は期間の定めがなく毎月自動更新。1 年間の支払額 1,200 円（税込）は目安
- 理由: オーナー入力。氏名・住所・電話番号をリポジトリへ置かない。公開問い合わせ先は設定済み（アドレスは正本へ複製しない）
- 採用しなかった案: 特商法ページへ氏名・住所・電話番号を掲載する。料金を変える
- 結果: 管轄は東京地方裁判所・東京簡易裁判所。法務ページへ課税 / 免税の推測は書かない。特商法に追加料金・送料なしの説明を置く。購入直前に支払時期 / 提供時期 / 解約条件を表示する。ブラウザ実機確認済み

## D140. 税務 / 会計は運用確認済みとし、課税区分は断定しない

- 状態: 採用（M11.7 COMPLETE 時の運用確認）
- 判断: インボイス発行事業者としては未登録。公開表示は月額 100 円（税込）を維持する。会計運用では Stripe の入金額だけを売上とせず、売上総額 / Stripe 手数料 / 実際の入金額を区分して追跡・記録する。免税事業者であるとは断定しない。課税事業者該当性は売上規模等に応じて継続確認する
- 理由: 公開前の運用確認として必要な範囲。法務ページへ課税 / 免税の推測を書かない
...
- 課税事業者該当性は売上規模等に応じて継続確認する（免税事業者であるとは断定しない。公開ブロッカーではない）
