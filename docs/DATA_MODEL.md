...
M1 のアプリ実装は、この定義に従う。M2・M3・M5.1 でもこの項目は増やさない。
...
| 値 | 意味 |
|---|---|
| `drag` | ドラッグで矩形を描いて登録する |
| `stamp` | テンプレートサイズの候補を、クリックで位置指定する |

- テンプレートが無いときは `drag` のみ
- 最初の 1 件の成功後、自動では `stamp` に切り替えない

#### PanelCandidate

`stamp` 中の未確定矩形。Panel Store には入れない。
...
- PDF 上のクリックは位置の指定・移動だけに使う。クリック位置を矩形の中心とする
- ページ端では、矩形全体が 0〜1 に収まるようクランプする
- 候補そのもののクリックでは確定しない
- 確定は「この位置で登録」だけとする
- 破棄は「やめる」または Esc。破棄後もテンプレートは残す
