...
- 採用しなかった案: ファイルのドラッグ＆ドロップ投入、ズーム、サムネイル、カット枠の手置きを M0 に含める
- 結果: M0 では解析・尺・タイムライン・動画のコードも空ファイルも作らない。Panel 操作は M1 で足す

## D5. 実行時データと将来データ境界を分ける

- 状態: 採用
- 判断: M0 は `PdfSession`、M1 は `Panel` を現行データとする。Storyboard Data → Cut Data → Timeline → Rush は境界だけを記す
- 理由: 詳細モデルを先に決めると、未検証の解析前提が仕様に混ざる
- 採用しなかった案: 最初から Storyboard Data / Cut / Timeline のスキーマを定義する
...
- 結果: `pdf-loader.js` と `pdf-viewer.js` は PDF 専用のままにする。Panel は `panel-store.js` と `panel-overlay.js` を新設して扱う
...
- 結果: ポインタ座標はオーバーレイの表示矩形から相対値へ変換する。枠の配置はパーセント指定を使う
...
- 結果: M1 では自動検出も `confidence` 計算も実装しない。`confidence` フィールドも持たない
...
- 結果: ソート UI とフィルタ UI は M1 に入れない。現在ページの行は見た目で区別する

## D11. Panel は保存しない

- 状態: 採用（M1）
- 判断: メモリ上のみ。新しい PDF の読み込み成功時に破棄する
- 理由: M1 の目的は手動登録の操作を成立させることであり、プロジェクト保存ではない
- 採用しなかった案: localStorage、IndexedDB、JSON エクスポート
- 結果: 読み込み失敗で直前 PDF を維持する場合は、Panel も残す

## 未決

- ライセンス
- GitHub Pages の公開 URL / リポジトリ公開範囲
- M2 以降の具体的な手段（人手入力か、画像処理か、など）
