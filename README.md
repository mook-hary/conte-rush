...
**実装済み（M5.1）** は、多数の Cut を扱いやすくするための UI / 操作改善です。ページ送り位置、高密度 Cut 一覧、入力クリア、Panel の stamp 登録を含みます。

**次の仕様（M5.2）** は、既存 Cut の編集導線を明確にし、Timeline の `startFrame` を横バーのドラッグで編集することです。M5.2 のアプリ実装はまだありません。仕様は [docs/SPEC.md](docs/SPEC.md) を正とします。

MP4 出力は未実装です。

## 現状できること（M0〜M5.1・実装済み）
...
- CUT番号と尺の個別クリア
- 前回サイズを使った Panel の stamp 登録

## 次の仕様（M5.2・アプリ未実装）

- 新規 Cut と既存 Cut 編集を別フォームで区別する
- 選択中 Cut の番号・尺を「変更を保存」で更新する
- 横 Timeline で配置済み Panel の `startFrame` をドラッグ編集する

M5.2 では Panel / Cut / Timeline / Rush の保存構造と、Rush 再生ロジックは変えません。

## 現状できないこと

次は構想段階であり、実装していません。M5.2 の対象でもありません。
...
## 使い方（M0 / M1 / M2 / M3 / M4 / M5 / M5.1。M5.2 は未実装）
...
8. Cut の「Timeline」から開始フレームを入力する
